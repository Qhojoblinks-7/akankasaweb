...
function DictionaryEntry({ entry, currentDialect }) {
  if (!entry || !entry.translations || !entry.translations[currentDialect]) {
    return <p className="text-red-500">No entry found for the selected dialect.</p>;
  }

  const { headword, translations, pronunciation, partOfSpeech, definitions, examples, relatedWords, akanDialects } = entry;
  ...
  return (
    <div className="mb-6 p-5 border border-gray-300 rounded-lg shadow-md bg-white">
      ...
      {/* Definitions */}
      <div className="mb-4">
        <h4 className="text-lg font-semibold text-gray-800 mb-1">Definition (English):</h4>
      ...
      {examples[currentDialect] && (
        <div className="mb-4">
          <h4 className="text-lg font-semibold text-gray-800 mb-1">Examples ({currentDialect}):</h4>
          <ul className="list-disc list-inside text-gray-700">
      ...
      {/* Related Words */}
      {relatedWords[currentDialect] && (
        <div className="mb-4">
          <h4 className="text-lg font-semibold text-gray-800 mb-1">Related Words ({currentDialect}):</h4>
      ...
      <div className="mt-4 text-sm text-gray-500">
        <p>Dialect: <span className="font-medium">{currentDialect}</span></p>
