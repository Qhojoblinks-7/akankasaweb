...
const DashboardMobileTemplate = ({
  ...
  const toggleMobileMenu = useCallback(() => setIsMobileMenuOpen(prevState => !prevState), []);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <button
        ...
        className="lg:hidden fixed top-4 right-4 bg-black text-white rounded-md p-2 z-50 focus:outline-none"
      >
      ...
      {/* Sidebar */}
      <aside
        className={`bg-black mt-7 text-white w-64 p-4 flex flex-col fixed top-0 left-0 h-full z-40 transform transition-transform duration-300 ease-in-out ${
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 mt-10 lg:pl-64">
        <div className="lg:flex gap-4">
          {/* Left Section (Proverb, Festivals, Content Options) */}
          <section className="w-full lg:w-1/4 flex flex-col mb-4 lg:mb-0">
          ...
          {/* Right Section (Overview and Module Details) */}
                      ...
                      <div key={subtopic.id} className="relative mb-2 p-3 rounded-md bg-gray-100">
                        ...
                        <div className={`mt-2 overflow-hidden transition-all duration-300 ${isSubtopicVisible[subtopic.id] ? 'max-h-20 p-2' : 'max-h-0'}`}>
                          ...
                          {subtopic.readings?.map(reading => (
                            <p key={reading.id} className="text-gray-600 flex items-center mb-7">
                              <BookOpenIcon className="h-4 w-4 mr-1 text-gray-600 mb-7" fill="none" stroke="currentColor" strokeWidth="2" />
                              <Link
...
export default DashboardMobileTemplate;